...
#
# Usage:
#   ./tests/test-search.sh              # Run all tests
#   ./tests/test-search.sh --quick      # Health + basic search only
# =============================================================================
...
searxng_curl() {
    docker exec -e FETCH_URL="$1" -e FETCH_TIMEOUT="$REQUEST_TIMEOUT" \
        "$SEARXNG_CONTAINER" python3 -c "
import urllib.request, os, sys
try:
    req = urllib.request.Request(os.environ['FETCH_URL'])
    resp = urllib.request.urlopen(req, timeout=int(os.environ.get('FETCH_TIMEOUT', '30')))
...
SQUID_LOG="$SCRIPT_DIR/../logs/squid/access.log"
